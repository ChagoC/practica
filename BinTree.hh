...
class BinTree {
  ...
  BinTree(shared_ptr<Node> p) : p(p) {}

  // Notes:
  //   - default operator=() is good.
  //   - default destructor is good. Θ(n) where n is the number of nodes in the
  //   tree.
  //   - std::swap() already works by default.

  int inputformat, outputformat;
  static void checkFormat(int format) {
    if (format == POSTORDERFORMAT or format == LEFTVISUALFORMAT or
        format == VISUALFORMAT or format == INLINEFORMAT)
      return;
  ...
  static void readVisualFormatRec(const vector<string> &v, int i, int j,
                                  BinTree<T> &t);
 ...
 public:
  ...
  BinTree() : p(nullptr) { inputformat = outputformat = INLINEFORMAT; }
  ...
  // Pre: true
  // Post: Sets inputformat and outputformat of implicit tree and its
  // descendants to 'format'
  ...
  int getInputFormat() const { return inputformat; }
  ...
  // writing operator <<
  /* Pre: true */
  /* Post: a representation of t is written on os deppending on current value of
   * attribute 'outputformat' */
  template <class U>
  friend std::ostream &operator<<(std::ostream &os, const BinTree<U> &t);

  // reading operator >>
  // Pre: 'is' contains a representation of a binary tree deppending on current
  // value of attribute 'inputformat'. Post: t is assigned the tree value read
  // from 'is'
  template <class U>
...
string BinTree<T>::value2string(const T &value) {
  ...
  if (not(mycout << value))
    dieInputOutputBinTree(
        "Error: We could not transform a value into a string");
  string s = mycout.str();
  if (int(s.size()) == 0)
    dieInputOutputBinTree(
        "Error: length 0 is not allowed for strings describing values");
  for (int i = 0; i < int(s.size()); ++i)
    if (s[i] == ' ')
      dieInputOutputBinTree(
          "Error: white spaces are not allowed for strings describing values");
  return s;
}
...
void BinTree<T>::string2value(const string &s, T &value) {
  ...
  if (not(mycin >> value))
    dieInputOutputBinTree(
        "Error: We could not transform a string into a value");
  string rest;
  if (mycin >> rest)
    dieInputOutputBinTree(
        "Error: We could not transform a string into a value");
...
void BinTree<T>::insertPrefixVectorString(string prefix, vector<string> &v) {
  for (int i = 0; i < int(v.size()); ++i) v[i] = prefix + v[i];
}
...
template <class T>
void BinTree<T>::writeBinTreePostOrderRec(std::ostream &os,
                                          const BinTree<T> &t) {
  if (t.empty()) return;
...
void BinTree<T>::readStringTree(
  ...
  if (itoken >= int(vtoken.size()) or vtoken[itoken].first != ",") {
    ...
    if (itoken < int(vtoken.size()) and vtoken[itoken].first == ")") itoken++;
    return;
  }
  itoken++;
  BinTree<T> right;
...
void BinTree<T>::readStringTree(const std::string &s, BinTree<T> &t) {
  ...
  for (int i = 0; i < int(s.size()); i++) {
    if (s[i] == ',' or s[i] == '(' or s[i] == ')') {
      if (iant < i)
        vtoken.push_back(std::pair<std::string, std::string>(
            "value", s.substr(iant, i - iant)));
      iant = i + 1;
      vtoken.push_back(
          std::pair<std::string, std::string>(std::string(1, s[i]), ""));
    }
  }
  if (iant < int(s.size()))
    vtoken.push_back(
        std::pair<std::string, std::string>("value", s.substr(iant)));
  int itoken = 0;
...
template <typename T>
void BinTree<T>::readVisualFormatRec(const vector<string> &v, int i, int j,
                                     BinTree<T> &t) {
  if (i < 0 or int(v.size()) <= i or j < 0 or int(v[i].size()) <= j or
      v[i][j] == ' ') {
    t = BinTree<T>();
    return;
  }
  ...
  int jdown = jini;
  while (jdown < int(linedown.size()) and jdown <= jend and
         linedown[jdown] == ' ')
    jdown++;
  ...
  int jleft = jdown - 1;
  BinTree<T> left;
  if (jleft >= 0 and jleft < int(line2down.size()) and
      line2down[jleft] == '-') {
    while (jleft >= 0 and line2down[jleft] == '-') jleft--;
    checkFormatCondition(0 <= jleft and jleft < int(line3down.size()) and
                         line3down[jleft] == '|');
  ...
  if (jright < int(line2down.size()) and line2down[jright] == '-') {
    while (jright >= 0 and jright < int(line2down.size()) and
           line2down[jright] == '-')
      jright++;
    checkFormatCondition(0 <= jright and jright < int(line3down.size()) and
                         line3down[jright] == '|');
...
vector<string> BinTree<T>::generateVisualFormatRec(const BinTree<T> &t) {
  ...
  if (int(leftresult.size()) == 0) {
    ...
    result.push_back(string(rootpos + 1, ' ') +
                     string(rightrootpos - rootpos - 1, '-'));
  ...
  if (int(rightresult.size()) == 0) {
    ...
    result.push_back(string(rootpos - rootrelativepos, ' ') + rootstring);
    result.push_back(string(rootpos, ' ') + string(1, '|'));
    result.push_back(string(leftrootpos + 1, ' ') +
                     string(max(0, rootpos - leftrootpos - 1), '-'));
  ...
  for (int i = 0; i < int(leftresult.size()) and i < int(rightresult.size());
       ++i)
    addspace = max(addspace, int(leftresult[i].size()) -
                                 firstNonBlankPosInString(rightresult[i]) +
                                 MINSPACEBETWEENSUBTREES);
  ...
  result.push_back(string(rootpos, ' ') + string(1, '|'));
  result.push_back(string(leftrootpos + 1, ' ') +
                   string(rootpos - leftrootpos - 1, '-') + string(1, ' ') +
                   string(max(0, rightrootpos - rootpos - 1), '-'));
  result.push_back(string(leftrootpos, ' ') + string(1, '|') +
                   string(rightrootpos - leftrootpos - 1, ' ') +
                   string(1, '|'));
  // cout << "Adding left and right result" << endl;
  for (int i = 0; i < int(leftresult.size()) or i < int(rightresult.size());
       ++i) {
    if (i < int(leftresult.size()) and i < int(rightresult.size())) {
      ...
      for (int j = 0; j < size; ++j) {
        if (j < leftsize and leftline[j] != ' ')
          line[j] = leftline[j];
        else if (j < rightsize and rightline[j] != ' ')
          line[j] = rightline[j];
      }
...
std::istream &operator>>(std::istream &is, BinTree<T> &t) {
  ...
  vector<string> v;
  string line;
  while (getline(is, line) and line != "") v.push_back(line);
